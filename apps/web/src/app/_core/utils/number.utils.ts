...
export function getShortNumber(input: any, decimals?: any) {
	...
	if (!isNegativeValues) {
		...
		return (input * -1 / Math.pow(1000, exp)).toFixed(decimals) + suffixes[exp - 1];
	}
}
...
export function numberToCurrency(num: number, config: { locale?: string, currencySymbol?: string, currencyCode?: string, digitsInfo?: string }) {
	return formatCurrency(num, config.locale || 'en-US', config.currencySymbol || '$', config.currencyCode || 'USD', config.digitsInfo || '1.0-2');
}
