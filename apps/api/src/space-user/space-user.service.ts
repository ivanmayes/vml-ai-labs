...
export class SpaceUserService {
	...
	public async findSpaceUsers(
		...
		if(query && query.trim()) {
			qb.andWhere('(LOWER(user.email) LIKE LOWER(:query) OR LOWER(user.firstName) LIKE LOWER(:query) OR LOWER(user.lastName) LIKE LOWER(:query))', {
				query: `%${query}%`
			});
		}
	...
	public async addUserToSpaceByEmail(spaceId: string, email: string, role: SpaceRole, organizationId: string) {
		...
		if(!user) {
			...
			if(!authStrategy) {
				throw new Error('No authentication strategy found for organization');
			}
	...
	public async addUserToSpace(spaceId: string, userId: string, role: SpaceRole) {
		// Check if user is already in the space
		const existing = await this.spaceUserRepository.findOne({
	...
	public async updateUserRole(spaceId: string, userId: string, role: SpaceRole) {
	...
	public async getUserSpaceRole(spaceId: string, userId: string): Promise<SpaceRole | null> {
		const spaceUser = await this.spaceUserRepository.findOne({
			where: { spaceId, userId }
		});
	...
	public async isUserSpaceAdmin(spaceId: string, userId: string, userRole: UserRole): Promise<boolean> {
	...
	public async hasSpaceAccess(spaceId: string, userId: string, userRole: UserRole, isSpacePublic: boolean): Promise<boolean> {
		// Platform admins and super admins have automatic access
		if(userRole === UserRole.SuperAdmin || userRole === UserRole.Admin) {
			return true;
		}
