...
export class UserConsole {
	...
	public async installUser(organization?: Organization) {
		...
		console.log(Utils.formatMessage(`Installing user to ${organization.id}`, ErrorLevel.Info));
		console.log(
			Utils.formatMessage(`Looking for Authentication Strategies...`, ErrorLevel.Info)
		);
		const authStrategies: AuthenticationStrategy[] = await this.authenticationStrategyService
			.find({
				where: {
					organizationId: organization.id
				}
		...
		if(authStrategies?.length) {
			console.log(`Which Authentication Strategy should this User belong to?`.bold);
			console.log('\tPlease make a selection:');
			...
			const orgResponse = await Utils.getUserResponse('\tAuthentication Strategy Number: ');
			const idx = parseInt(orgResponse);
			...
			console.log(
				Utils.formatMessage(`No Authentication Strategies found.`, ErrorLevel.Warning)
			);
			console.log(`Create new Authentication Strategy?`.bold);
			const shouldCreate = await Utils.getUserResponse(`\t[y/${' N '.bgWhite.black.bold}]: `);
		...
		user.authenticationStrategyId = authStrategy.id;
		...
		if(!FraudPrevention.Forms.Validation.Validate.email(email)) {
			...
			user.email = email;
			user.emailNormalized = FraudPrevention.Forms.Normalization.normalizeEmail(email);
		}
		...
		if(!savedUser) {
			throw Utils.formatMessage(`Error saving new user.`, ErrorLevel.Error);
		}

		console.log(Utils.formatMessage(`Success! User created with id: ${user.id}`));
