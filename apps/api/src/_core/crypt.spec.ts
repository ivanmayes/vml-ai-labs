...
describe('Crypt', () => {
	describe('get random number', () => {
		it('should be between 0 and 1', async () => {
			expect(Crypt.random())
				.toBeWithinRangeExclusive(0, 1);
		});
	});

	describe('encryption', () => {
		it('should create a valid SHA256 hash', () => {
			expect(Crypt.createSHA256Hash('A_T0tally_Fake_Signing_K#y', 'SomeUser@email.com'))
		...
		it('should be encrypted', () => {
			...
			expect(Crypt.encrypt(testData, signingKey, iv))
				.toBe('wsIyLLkq1bb6pQ99BweMnpwcWeeNzluaaPbTTVEBvhv6RnTVXUAgZNUtd9+fIFSm');
		});
		...
		it('should be decrypted', () => {
			...
			expect(Crypt.decrypt(testData, signingKey, iv))
				.toBe('This is only a test. No cause for alarm.');
		});
		...
		it('should not be decrypted', () => {
			...
			expect(Crypt.decrypt(testData, signingKey, iv))
				.not.toBe('This is only a test. No cause for alarm.');
		});
	});
});
