import { ValidationOptions, registerDecorator, buildMessage, ValidationArguments } from 'class-validator';
...
export function IsFormResult(options?: FormResultOptions, validationOptions?: ValidationOptions) {
		...
		registerDecorator({
			...
			validator: {
				defaultMessage: buildMessage(
					function(eachPrefix: string, args: ValidationArguments) {
						return '$property must be Array<FieldResult> ({ slug: string, value: string | number | boolean | FieldResult[] })';
					},
					validationOptions
				),
