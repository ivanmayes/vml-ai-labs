...
export class S3 {
	...
	public static async upload(
		...
		const response = await s3.putObject(file).promise()
			.catch(err => {
				if(this.isDebug) {
					console.log(err);
				}
		...
		if(response) {
			...
			return {
				...
				path: 'https://' + this.s3Config.bucketName + '.s3.amazonaws.com/' + folderName + fileName
			};
		} else {
	...
	public static async getContents(folderName, continuationToken) {
		AWS.config.update({
			accessKeyId: this.s3Config.accessKeyId,
			secretAccessKey: this.s3Config.secretAccessKey
		});
