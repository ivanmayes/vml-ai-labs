...
export class Cognito {
	...
	private static cognitoConfig = {
		...
		jwk: (process.env.AWS_COGNITO_JWK_JSON)
			?
				JSON.parse(Buffer.from(process.env.AWS_COGNITO_JWK_JSON, 'base64').toString()) as JWKS
				:
				null
	};

	public static async signUp(username: string, password: string, userAttributes: UserAttribute[]) {
		const cisp = this.getCISP();

		let error: AWSError;
		const response: CognitoIdentityServiceProvider.SignUpResponse = await cisp.signUp({
	...
	public static async verify(username: string, confirmationCode: string) {
		...
		let error: AWSError;
		const response: CognitoIdentityServiceProvider.ConfirmSignUpResponse = await cisp.confirmSignUp({
			ClientId: this.cognitoConfig.appClientId,
	...
	public static async signIn(username: string, password: string) {
		...
		let error: AWSError;
		const response: CognitoIdentityServiceProvider.InitiateAuthResponse = await cisp.initiateAuth({
	...
	public static async refreshToken(idToken: string, refreshToken: string) {
		...
		const userData: any = await this.decodeToken(idToken)
			.catch(err => {
				console.log(err);
			});
