...
export class WPPOpen {
 ...
 public static async getWorkspaceAncestor(token: string, workspaceId: string, ancestorId: string): Promise<WorkspaceHierarchy> {
  if(!WPPOpen.config?.host) {
   throw new Error('WPPOpen host is not set.');
  }
 ...
 private static getAncestorById(hierarchy: WorkspaceHierarchy[], ancestorId: string): WorkspaceHierarchy {
